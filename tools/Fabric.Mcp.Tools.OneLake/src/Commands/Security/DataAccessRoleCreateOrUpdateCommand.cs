...
[CommandMetadata(
    ...
    Description = "Create a new OneLake data access role or replace an existing one. Optionally pass `etag` for optimistic concurrency on updates. Requires `OneLake.ReadWrite.All`. **Role name validation**: must start with a letter and contain ONLY letters and numbers (no underscores, hyphens, or other punctuation - Fabric returns 400 RequestBodyValidationFailed otherwise). Canonical body shape (Fabric REST `Create or update Data Access Role`): `{ \"decisionRules\": [ { \"effect\": \"Permit\", \"permission\": [ { \"attributeName\": \"Action\", \"attributeValueIncludedIn\": [\"Read\"] }, { \"attributeName\": \"Path\", \"attributeValueIncludedIn\": [\"/Files/your/path/\"] } ] } ], \"members\": { \"microsoftEntraMembers\": [ { \"objectId\": \"<entra-object-guid>\", \"tenantId\": \"<entra-tenant-guid>\" } ] } }`. Body notes: (a) `permission` MUST contain BOTH an `Action` attribute and a `Path` attribute (Fabric returns `PolicyValidationError: Array 'permission' must have 2 elements` otherwise); (b) `tenantId` MUST be a valid GUID (not a placeholder); (c) `fabricItemMembers` is also a valid member kind; (d) replace, not patch - the supplied body fully overwrites the role.",
    Destructive = false,
    Idempotent = true,
    LocalRequired = false,
    OpenWorld = false,
