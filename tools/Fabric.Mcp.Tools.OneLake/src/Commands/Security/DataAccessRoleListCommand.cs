...
[CommandMetadata(
    Id = "b4f4b0b9-4ee1-4eaa-9786-58f5b2d92dde",
    Name = "list_data_access_roles",
    Title = "List OneLake Data Access Roles",
    Description = "List all OneLake data access roles defined on an item, including their decision rules and member principals. Requires `OneLake.Read.All`.",
    Destructive = false,
    Idempotent = true,
    LocalRequired = false,
    OpenWorld = false,
...
public sealed class DataAccessRoleListCommand(
    ...
    IFabricApiService fabricApiService) : GlobalCommand<DataAccessRoleListOptions>()
{
    ...
    public override async Task<CommandResponse> ExecuteAsync(CommandContext context, ParseResult parseResult, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var data = await _fabricApiService.ListDataAccessRolesAsync(workspace, item, options.ContinuationToken, cancellationToken);
            var result = new DataAccessRoleListCommandResult { Workspace = workspace, Item = item, Roles = data };
            context.Response.Results = ResponseResult.Create(result, OneLakeJsonContext.Default.DataAccessRoleListCommandResult);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error listing data access roles.");
            HandleException(context, ex);
        }
        return context.Response;
    }

    public sealed class DataAccessRoleListCommandResult
    {
